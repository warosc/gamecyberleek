...
export class StatusHud {
  ...
  constructor(
    ...
    this.energyText = scene.add.text(247, 88, t('hud.dashEnergy'), {
    ...
    const inventoryX = mobile ? GAME_WIDTH - 108 : GAME_WIDTH - 390;
    const inventoryButton = scene.add.rectangle(inventoryX, 42, mobile ? 68 : 72, mobile ? 52 : 44, 0x06101d, 0.88)
      .setStrokeStyle(2, 0x73ef62, 0.75).setInteractive({ useHandCursor: true }).setName('hud-inventory');
    const inventoryLabel = scene.add.text(inventoryX, 42, 'INV', {
      fontFamily: 'Arial Black', fontSize: '13px', color: '#73ef62',
    }).setOrigin(0.5).setInteractive({ useHandCursor: true });
    inventoryButton.on('pointerup', onInventory);
    inventoryLabel.on('pointerup', onInventory);
    ...
    this.xpLabel = scene.add.text(GAME_WIDTH / 2, GAME_HEIGHT - 22, t('hud.xpStart'), {
      fontFamily: 'Arial Black', fontSize: '10px', color: '#eaffff',
    }).setOrigin(0.5);
  ...
  update(delta: number, survivalMs: number, dashCharge: number) {
    ...
    this.phase.setText(td(runPhase(survivalMs).label));
    ...
    this.energyText.setText(dashCharge >= 1 ? t('hud.dashReady') : t('hud.dashCharge', { percent: Math.round(dashCharge * 100) }));
  }

  setHealth(current: number, max: number, damageFlash: Phaser.GameObjects.Rectangle) {
    const ratio = current / max;
    this.hp.setText(t('hud.hp', { current: Math.ceil(current), max })).setColor(ratio < 0.3 ? '#ff476f' : '#eaffff');
  ...
  setExperience(xp: number, level: number) {
    ...
    this.xpLabel.setText(t('hud.xp', { xp: Math.floor(xp), next: xpForLevel(level), level }));
  }
