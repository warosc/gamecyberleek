...
export class MinibossBanner {
  ...
  constructor(private readonly scene: Phaser.Scene) {
    ...
    const name = scene.add.text(GAME_WIDTH / 2, 158, 'REM-Ω // CENTINELA REMOLACHA', {
      fontFamily: 'Arial Black', fontSize: '11px', color: '#ffd7e5', letterSpacing: 2,
    }).setOrigin(0.5);
    this.panel = scene.add.container(0, 0, [back, this.fill, name]).setVisible(false);
  }
  ...
  show() {
    ...
    const plate = this.scene.add.rectangle(0, 0, 680, 92, 0x160811, 0.95).setStrokeStyle(3, 0xff3b76);
    const title = this.scene.add.text(0, -13, 'REM-Ω // BIOCENTINELA', {
    ...
    banner.add([plate, title, brief]).setAlpha(0).setScale(0.86);
